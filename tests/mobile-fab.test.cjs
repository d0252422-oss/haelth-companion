...
assert.match(html, /id="mood-options"/);
assert.match(html, /setChoiceValue\("mood-options",record\?\.moodScore\?\?3\)/);
assert.match(html, /moodScore:score\("mood-options"\)/);
assert.doesNotMatch(html, /moodScore:score\("energy-options"\)/);
assert.match(html, /sequence!==weightFormLoadSequence\|\|document\.getElementById\("weight-date"\)\.value!==date/);
assert.match(html, /sequence!==checkinFormLoadSequence\|\|document\.getElementById\("checkin-date"\)\.value!==date/);
assert.match(html, /weightFormLoadedDate="";document\.getElementById\("weight-record-id"\)\.value=""/);
assert.match(html, /checkinFormLoadedDate="";document\.getElementById\("checkin-status-id"\)\.value=""/);
assert.match(html, /weightFormLoadedDate!==selectedDate/);
assert.match(html, /checkinFormLoadedDate!==date/);
assert.match(html, /weightFormLoadedDate===date\)setSubmitting\(save,false\)/);
assert.match(html, /checkinFormLoadedDate===date\)setSubmitting\(save,false\)/);

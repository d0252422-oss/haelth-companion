...
test('tester entry exposes Android distribution and fail-closed iOS state', () => {
  assert.match(html, /0\.1\.0-beta\.5/u);
  assert.match(html, /releases\/download\/android-beta-v0\.1\.0-beta\.5\/health-sync-companion-beta-0\.1\.0-beta\.5-debug\.apk/u);
  assert.match(html, /55530b4605b8/u);

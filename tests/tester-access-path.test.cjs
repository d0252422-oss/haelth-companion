...
test('distribution config and Shortcut manifest contain no credential or fabricated URL', () => {
  ...
  assert.match(beta.ANDROID_BETA_APK_URL, /^https:\/\/github\.com\/d0252422-oss\/haelth-companion\/releases\/download\/android-beta-v0\.1\.0-beta\.10\//u);
  assert.equal(beta.ANDROID_BETA_APK_VERSION, '0.1.0-beta.10-debug');
  assert.equal(beta.ANDROID_BETA_APK_SIZE_BYTES, 23637477);

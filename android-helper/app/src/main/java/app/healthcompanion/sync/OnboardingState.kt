...
object OnboardingStateMachine {
    fun transition(current: ConnectorUiState, event: ConnectorEvent): ConnectorUiState = when (event) {
        ...
        ConnectorEvent.SYNC_STARTED -> if (current in setOf(ConnectorUiState.READY_TO_SYNC, ConnectorUiState.BACKGROUND_SYNCING, ConnectorUiState.SYNC_SUCCESS, ConnectorUiState.SYNC_PARTIAL, ConnectorUiState.SYNC_TIMEOUT, ConnectorUiState.SYNC_NO_DATA, ConnectorUiState.SYNC_ERROR)) ConnectorUiState.SYNCING else current
